...
def main(args, seed, save_path):
    ...
    inner_radius = (delano.y[delano.idx, -2] * delano.scale).item()
    material = args.MAT
    try:
    ...
    roc1 = (delano.elem_effl[delano.idx, 0] * delano.scale) ** -1 * (Q + n / (n - 1))
    roc2 = (delano.elem_effl[delano.idx, 0] * delano.scale) ** -1 * (Q + 1)
    ...
    sys.system.append(lens.Qbfs(
        ...
        aperture = 'obsc',
        min_r = [0.] * sys.sys_num,
        max_r = [inner_radius] * sys.sys_num,
    ))
    sys.system.append(lens.Qbfs(
        radius = [[(delano.elem_radius[delano.idx, 0] * delano.scale).item()]] * sys.sys_num,
        material = ['VACUUM'] * sys.sys_num,
        roc = (roc2 ** -1).tolist(),
        thick = [[(delano.elem_dist[delano.idx, 0] * delano.scale).item()]] * sys.sys_num,
        conic = [0.] * sys.sys_num,
        qi_list=[[0.] * sys.sys_num] * qorder,
        rnorm=[(delano.elem_radius[delano.idx, 0] * delano.scale).item()] * sys.sys_num,
        aperture = 'obsc',
    ...
    sys.system.append(lens.Qbfs(
        radius = [[(delano.elem_radius[delano.idx, 1] * delano.scale).item()]] * sys.sys_num,
        ...
        conic = [0.] * sys.sys_num,
        qi_list=[[0.] * sys.sys_num] * qorder,
        rnorm=[(delano.elem_radius[delano.idx, 1] * delano.scale).item()] * sys.sys_num,
        aperture = 'circ',
        min_r = [inner_radius] * sys.sys_num,
        max_r = [(delano.elem_radius[delano.idx, 1] * delano.scale).item()] * sys.sys_num,
    ))
    sys.system.append(lens.Qbfs(
        radius = [[inner_radius]] * sys.sys_num,
        material = ['MIRROR'] * sys.sys_num,
        roc = [2 * delano.elem_effl[delano.idx, 2] * delano.scale] * sys.sys_num,
        thick = [[delano.elem_dist[delano.idx, 2] * delano.scale]] * sys.sys_num,
        conic = [0.] * sys.sys_num,
        qi_list=[[0.] * sys.sys_num] * qorder,
    ...
    @torch.no_grad()
    def update():
        ray = sys.sample_ray_2d(args.SAMP_RAYS)
        ...
        inner_radius = torch.where(ray.valid, lens.length(o[-1, :, :, :, :, :, :, 0:2]), -torch.inf).amax(dim=[0, 2, 3, 4, 5]) # [sys]
        surfs[1].max_r = inner_radius
        surfs[2].max_r = inner_radius
        surfs[3].min_r = inner_radius
        surfs[3].max_r = torch.where(ray.valid, lens.length(o[-2, :, :, :, :, :, :, 0:2]), -torch.inf).amax(dim=[0, 2, 3, 4, 5]) # [sys]
        surfs[3].radius = torch.where(ray.valid, lens.length(o[-2, :, :, :, :, :, :, 0:2]), -torch.inf).amax(dim=[0, 2, 3, 4, 5])[:, None].repeat(1, sys.cfg_num)
        surfs[4].radius = inner_radius[:, None].repeat(1, sys.cfg_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='gen_cake')
