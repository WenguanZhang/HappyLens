...
# %%
def find_dng_with_glob(root_folder):
    return glob.glob(
        os.path.join(root_folder, '**/*.dng'), 
        recursive=True
    )
# %%
raw_data_path = r'xxxxxxxxx'
dng_list = find_dng_with_glob(raw_data_path)
# %%
train_count = 0
...
# %%
loop = tqdm(enumerate(dng_list), total=len(dng_list))
for i, img_path in loop:
    raw = rawpy.imread(img_path)
    if raw.color_desc == b'RGBG':
        raw_img = raw.raw_image_visible
        if raw_img.shape[0] >= train_img_size[0] and raw_img.shape[1] >= train_img_size[1]:
            # 1. read raw data
            ...
            if raw_img.shape[0] >= valid_img_size[0] and raw_img.shape[1] >= valid_img_size[1]:
