...
def main(args, seed, save_path):
    ...
    result_folder = f'{save_path}/seed_{seed}'
    os.makedirs(result_folder)
    
    #!->3 zoom optimization
    zoom = lens.Zoom(group_structure=args.GROUP_STRUCTURE, group_type=args.GROUP_TYPE,
                     sys_num=args.DELANO_SYS, target_fov=args.MAX_VIEW, 
                     target_effl=args.MERIT['EFL']['target'], 
                     target_fno=args.MERIT['FNO']['target'], 
                     target_totr=args.MERIT['TOTR']['target'], 
                     target_bfl=args.MERIT['BFL']['target'],
                     FF_min_dist=args.MERIT['AIR_THICK']['FF_target'],
                     FM_min_dist=args.MERIT['AIR_THICK']['FM_target'],
                     MM_min_dist=args.MERIT['AIR_THICK']['MM_target'],
                     stop_pos=args.STOP_POS, stop_fix=args.STOP_FIX)
    zoom.optimize_SA()
    zoom.optimize(args.LR_DELANO, result_folder)
    ...
    sys = lens.System(wavelengths=args.WAVELENGTHS,
                      ...
                      vig=args.VIG,
                      zoom=[zoom, args.SURF_TYPE, args.MAT_TYPE, args.MAT_CATA, args.MERIT],
                      )
    analysis = lens.Analysis(sys)
    merit = lens.Generation_Zoom(system=sys, samp_rays=args.SAMP_RAYS)
    ...
    for es, epochs in enumerate(args.EPOCH):
        ...
        optimizer = torch.optim.Adam(merit.params_lr(args.LR_OPT, args.LR_SCALE), fused=True)
        ...
        for epoch in range(epochs):
            ...
            if count_view == 1: 
                merit.update_stop_radius(target_FNO, args.STOP_FIX)
                merit.update_system(rmax=args.MAX_RADIUS, avg_cfg=sys.zoom_type[-2][-1] == 'F')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='gen_zoom_2x')
