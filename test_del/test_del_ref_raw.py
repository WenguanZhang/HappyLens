...
if __name__ == '__main__':
    ...
    name = args.name
    args = lens.GetYaml(f'./lens_yaml/{name}_ref.yaml')
    lens.configure_material_catalog(getattr(args, 'MATERIAL_CATALOG', None))

    result_folder = f'./results/{name}_{args.DATASET}_ref_{str(time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()))}'
    os.makedirs(result_folder)
    writer = SummaryWriter(f'{result_folder}/log')
    img_show_num = 1
    
    shutil.copyfile(f'./lens_yaml/{name}_ref.yaml', f'{result_folder}/{name}_ref.yaml')
    ...
    sys = lens.System(wavelengths=args.WAVELENGTHS, waveweights=args.WAVEWEIGHTS, p_wvl=args.P_WAVE, max_view=args.MAX_VIEW, sys_num=args.SYS_NUM, cfg_num=args.CFG_NUM, pre_samp=args.PRE_SAMP, norm_views=args.NORM_VIEWS, azimuths=args.AZIMUTHS, file=f'./lens_json/{name}_ref.json')
